...
COINS = Object.keys(COINS).map(coin => {
  COINS[coin].unit = COINS[coin].isTradePath
    ? COINS[coin].unit.toUpperCase()
    : COINS[coin].unit;
  return COINS[coin];
});

const privateApi = zaif.createPrivateApi(
  ENV.access_key,
  ENV.secret_key,
...
const getAllCoinsTrades = Object.keys(COINS).map(coin =>
  publicApi.trades(`${COINS[coin].unit}_${COMPARATIVE_UNIT}`)
);
const getAllCoinsRate = Object.keys(COINS).map(coin =>
  publicApi.lastPrice(`${COINS[coin].unit}_${COMPARATIVE_UNIT}`)
);
const mergeCoinsAndInfo = info =>
  Object.keys(COINS).map((coin, index) => {
    const { rates, balances, trades } = info;
    return Object.assign({}, COINS[coin], {
      rate: rates[index].hasOwnProperty('last_price')
        ? rates[index].last_price
        : 0,
      trades: trades[index],
      balance: balances.hasOwnProperty(COINS[coin].unit)
        ? balances[COINS[coin].unit]
        : 0
    });
  });
...
axios
  .all([...getAllCoinsRate, ...getAllCoinsTrades, getBalance])
  .then(result => {
    const rates = result.slice(0, getAllCoinsRate.length);
    const trades = result.slice(
      getAllCoinsRate.length,
      getAllCoinsRate.length + getAllCoinsTrades.length
    );
    const balances = result.slice(
      getAllCoinsRate.length + getAllCoinsTrades.length
    )[0].funds;
    ...
    const comsa = coinsInfo.filter(coin => coin.type === 'comsa');

    const bitbarContent = coins =>
      coins.map((coin, index) => {
        const { name, image, unit, rate, balance, isTradePath, trades } = coin;

        const bids = trades
          .filter(trade => trade.trade_type === 'bid')
          .map(bid => bid.amount);
        const asks = trades
          .filter(trade => trade.trade_type === 'ask')
          .map(ask => ask.amount);
        ...
        return {
          text: `[${unit.toUpperCase()}] ¥${rate} (¥${prefix}${floatFormat(
            difference,
            3
          )}) / ¥${floatFormat(Number(rate) * Number(balance), 3)}`,
          color: difference < 0 ? ENV.colors.red : ENV.colors.green,
          href: `https://zaif.jp/trade${
            isTradePath ? '/' : '_'
          }${unit.toLowerCase()}_jpy`
        };
      });

    const currenciesContent = bitbarContent(currencies);
    const tokensContent = bitbarContent(tokens);
    const comsaContent = bitbarContent(comsa);
